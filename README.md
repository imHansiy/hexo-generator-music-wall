...
跨页播放器默认以圆形封面收起，鼠标悬浮或键盘聚焦时自动展开；控制器末尾的音乐墙按钮可直接返回完整音乐界面。
首次访问博客时，插件会读取歌单并默认选中第一首歌曲，因此悬浮播放器无需先进入音乐墙即可显示；浏览器不会未经用户操作自动播放。
...
当页面通过站内链接或主题 PJAX 切换时，插件会保留同一个音频实例并继续播放。浏览器地址栏直接输入新 URL、手动刷新或打开新标签页会销毁当前页面运行环境，只能恢复歌曲和进度，无法做到物理意义上的无缝续播。
